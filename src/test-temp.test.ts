...
function runChild(body: string): { paths: string[]; code: number; signal: string | null } {
  ...
  return { paths, code: res.status ?? -1, signal: res.signal };
}
...
test("tempDir returns a usable directory under tmpdir with the given prefix", () => {
  const { paths } = runChild(
    `const d = tempDir("yimbot-selftest-");
     console.log(d);
     console.log(String(require("node:fs").statSync(d).isDirectory()));`,
  );
  assert.equal(paths.length, 1);

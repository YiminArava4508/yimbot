...
# PR-fix sessions run unattended; uses the planning model if set and passes the
# implementation model through the environment for the pickup-ticket skill (both
# optional). Pure (reads only the environment); unit-tested via sourcing.
build_claude_cmd() {

...
test("settingRows renders the max-estimate cap, empty as no cap", () => {
  const display = (maxEstimate: string) =>
    settingRows({ ...sample, maxEstimate }, "x").find((r) => r.envKey === "MAX_ESTIMATE")!.display;
  assert.equal(display(""), "(no cap)");
  assert.equal(display("3"), "3");
});

test("validateDraft allows an empty max estimate but rejects non-positive values", () => {
  assert.deepEqual(validateDraft(setEdit(newDraft(sample), "MAX_ESTIMATE", "")), {});
  assert.deepEqual(validateDraft(setEdit(newDraft(sample), "MAX_ESTIMATE", "5")), {});
  assert.match(validateDraft(setEdit(newDraft(sample), "MAX_ESTIMATE", "0")).MAX_ESTIMATE, /positive integer/i);
  assert.match(validateDraft(setEdit(newDraft(sample), "MAX_ESTIMATE", "abc")).MAX_ESTIMATE, /positive integer/i);
});
